...
func (m *edgeIterMerge) Push(next func() (Edge, bool)) {
	heap.Push(&m.h, next)
}
func (m edgeIterMerge) Peek() (info Edge, ok bool) {
	if len(m.h) > 0 {
		info, ok = m.h[0].Info, true
	}
	return
}
func (m *edgeIterMerge) Next() (info Edge, ok bool) {
	h := m.h
	if len(h) == 0 {
		return
	}
